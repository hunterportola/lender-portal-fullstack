...
import { HttpCache, ReleaseCacheItem } from "./HttpCache.js";
import { throwError, ExceptionDispatcher, getError } from "../Exceptions/index.js";
import { GetClientConfigurationCommand, } from "../Documents/Operations/Configuration/GetClientConfigurationOperation.js";
...
export class RequestExecutor {
    ...
    _updateDatabaseTopologySemaphore = new Semaphore();
    _updateClientConfigurationSemaphore = new Semaphore();
    static _backwardCompatibilityFailureCheckOperation = new GetStatisticsOperation("failure=check");
    static _failureCheckOperation = new DatabaseHealthCheckOperation();
    ...
    */
    ...
    _topologyHeaderName = HEADERS.TOPOLOGY_ETAG;
    _lastServerVersion;
        ...
        try {
            if (!this.firstTopologyUpdatePromise) {
                if (!this._lastKnownUrls) {
                    // shouldn't happen
                    throwError("InvalidOperationException", "No known topology and no previously known one, cannot proceed, likely a bug");
                }
                if (!this._disableTopologyUpdates) {
    ...
    _throwExceptions(details) {
        throwError("InvalidOperationException", "Failed to retrieve database topology from all known nodes"
            + EOL + details);
    }
    static validateUrls(initialUrls, authOptions) {
        ...
        for (const url of initialUrls) {
            ...
            if (authOptions && authOptions.certificate) {
                throwError("InvalidOperationException", "The url " + url + " is using HTTP, but a certificate is specified, which require us to use HTTPS");
            }
            throwError("InvalidOperationException", "The url " + url
        ...
        try {
            ...
            if (response.status >= 400) {
                ...
                if (!unsuccessfulResponseHandled) {
                    ...
                    if (dbMissingHeader) {
                        throwError("DatabaseDoesNotExistException", dbMissingHeader);
                    }
    ...
    async _send(chosenNode, command, sessionInfo, request) {
        ...
        if (sessionInfo && sessionInfo.lastClusterTransactionIndex) {
            ...
            if (this._lastServerVersion && "4.1".localeCompare(this._lastServerVersion) > 0) {
                throwError("ClientVersionMismatchException", "The server on " + chosenNode.url + " has an old version and can't perform "
    ...
    async _handleUnsuccessfulResponse(chosenNode, nodeIndex, command, req, response, responseBodyStream, url, sessionInfo, shouldRetry) {
        ...
        switch (response.status) {
            ...
            case StatusCodes.Forbidden: {
                ...
                throwError("AuthorizationException", `Forbidden access to ${chosenNode.database}@${chosenNode.url}`
            ...
            case StatusCodes.Gone: {
                ...
                if (command.isFailedWithNode(chosenNode)) {
                    command.failedNodes.set(chosenNode, getError("UnsuccessfulRequestException", "Request to " + url + "(" + req.method + ") is not relevant for this node anymore."));
                }
