...
export interface DateInputProps {
  ...
  onValidDate?: () => void; // Callback when a valid date is entered
  className?: string;
...
const DateInput = React.forwardRef<HTMLInputElement, DateInputProps>(
  ({ 
    ...
    const shouldAutoTab = (input: string): boolean => {
      ...
      // Auto-tab if first digit is 4-9 (impossible month start)
      if (digits.length === 1 && parseInt(digits[0]) >= 4) {
        return true
      }
      
      // Auto-tab if we have a complete valid date (8 digits)
    ...
    const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Check if we should auto-tab
      if (shouldAutoTab(rawInput) && onValidDate) {
        onValidDate()
      }
    }
